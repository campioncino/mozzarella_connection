...
class StoricoReportConfezionamentoScreen extends StatefulWidget {
  const StoricoReportConfezionamentoScreen({Key? key}) : super(key: key);

  @override
  State<StoricoReportConfezionamentoScreen> createState() => _StoricoReportConfezionamentoScreenState();
}

class _StoricoReportConfezionamentoScreenState extends State<StoricoReportConfezionamentoScreen> {
  Categoria? _cat;
  ...
  _StoricoReportConfezionamentoScreenState() {
  ...
  //     } else {
  //       List<ReportConfezionamento> list = _values!.where((v) {
  ...
  Future<void> readData() async {
    _filteredValues.clear();
    WSResponse resp = await ReportConfezionamentoRestService.internal(context).getAllReportConfezionamento();
  ...
  Widget build(BuildContext context) {
    return  ScaffoldMessenger(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Storico Report Confezionamento'),
        ),
  ...
  Widget _buildChildRow(BuildContext context, ReportConfezionamento report, int position){
                ...
                child: InkWell(
                  ...
                  child: ListTile(
                    ...
                    title: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text("Trasporto per il giorno ${report.dataTrasporto.toString().substring(0,10)} ",style: TextStyle(fontWeight: FontWeight.bold,fontSize: 16),),
  ...
  void _goToDetail(ReportConfezionamento report){
    List data =jsonDecode(report.productsData!);
    Navigator.push(context, MaterialPageRoute(builder: (context)=> DettaglioReportScreen(data:data ,dtReport: report.dataTrasporto.toString().substring(0,10),isFromStorico: true,)));
  }
